# ---
# title: "Figure 5: Illustration of return levels for the NUTS-3 Osttirol, 
# Austria. Each column corresponds to a nonstationary model,
...
# for this NUTS3 area, three models are compared: GA, GG, GUM, GEV
n.model = 4

pdf("../Figures/Fig5_exFitWithoutZeros.pdf",width = 10,height = 8)
...
for(i.chain in 1:n.chain){
  ...
  for(i.model in 1:n.model){
    ...
    plot(df$gt,df$maxswe,pch=20,col="black",
         xlab="",ylab="",ylim=c(0,800),xaxt="n",yaxt="n",frame.plot = F)
    if(i.model==1){
...
mtext(text = c("GA","GG","GUM","GEV"),
      side = 3, at=seq(from=0.12,to=0.88,length.out=n.model), outer = T,line = 0.5)
