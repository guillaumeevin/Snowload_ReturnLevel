...
# read return levels
#===============================================================================
RL3vs1 = Qinf = Qsup = vector(length = n.nuts.alt)
for(inut in 1:n.nuts.alt){
  f = paste0("../ReturnLevelOutput/b",inut,".rds")
  if(file.exists(f)){
    ll = readRDS(file = f)
    RL = ll$bootout
    allRL = RL[,,3]/RL[,,1]-1
    Qinf[inut] = quantile(allRL,probs=0.05,na.rm = T)
    Qsup[inut] = quantile(allRL,probs=0.95,na.rm = T)
    RL3vs1[inut] = Qsup[inut]-Qinf[inut]
  }
}


#===============================================================================
# Fig_mapUncboot_RL3vs1.png
#===============================================================================
precip_5=c(rgb(199,234,229,maxColorValue=255),
...
colorscale = binned_pal(scales::manual_pal(precip_5))

# selected nuts/alt
allUnc = RL3vs1[dfNuts$i6584]*100
bnd = 100
mybreaks <- seq(from=0,to=bnd,by=20)
shp_nuts$rl = allUnc
mylabels = mybreaks
...
plt =  ggplot() +
  ...
  binned_scale(aesthetics = "fill",name="Rel. Diff. Unc. (%)",palette = colorscale,#na.value = "gray20",
               guide="coloursteps",limits=range(mybreaks),breaks=mybreaks,
               show.limits = T,oob=squish,labels= mylabels)
ggsave(filename = paste0("Fig10_mapUncboot_RL3vs1.png"), path = "../Figures/",
...
plt =  ggplot() +
  ...
  guides(fill=guide_colorbar(barwidth = 1.5, barheight = 15,
                             label.theme = element_text(size = 10, face = c("bold"),color=c("black")),
                             title.theme=element_text(size = 10, face = "bold"),title.position = "top"))+
  binned_scale(aesthetics = "fill",name="Rel. Diff. RL (%)",palette = colorscale,#na.value = "gray20",
               guide="coloursteps",limits=range(mybreaks),breaks=mybreaks,
               show.limits = T,oob=squish,labels= mylabels)
ggsave(filename = paste0("FigS5_mapUncRL3vs1_Qinf.png"), path = "../Figures/",
       device = "png",units="cm",height=12,width=18,dpi=300)
...
plt =  ggplot() +
  ...
  binned_scale(aesthetics = "fill",name="Rel. Diff. RL (%)",palette = colorscale,#na.value = "gray20",
               guide="coloursteps",limits=range(mybreaks),breaks=mybreaks,
               show.limits = T,oob=squish,labels= mylabels)
ggsave(filename = paste0("FigS6_mapUncRL3vs1_Qsup.png"), path = "../Figures/",
       device = "png",units="cm",height=12,width=18,dpi=300)